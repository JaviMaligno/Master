...
%	\end{tikzcd}
%	$$
	\scalebox{1}{$C_*(E_2)\xrightarrow{h. e.} C_*(|N(PaB)|)\to C_*(O_A)\leftarrow H_*(E_2) \leftarrow \encircled{G_\infty} \to \widetilde{\mathcal{B}}\xrightarrow{\cong}\mathcal{B}$}\pause %el último no es quasi-iso, es solo map pero nos sirve
	\vspace{1cm}
...
\begin{frame}
	\begin{itemize}
		\item<1-> %Taking a $\Q$-linear version of the nerve and constructing the associated chain complex we obtain 
		Define the chain operad $C_*(O_A)$ where $C_k(O_A(n))=A_n^{\otimes k}$ and
		$$
		d(a_1\otimes\cdots\otimes a_k)=\pi(a_1)a_2\otimes\cdots\otimes a_n-a_1a_2\otimes \cdots\otimes a_n+\cdots$$ $$\cdots+(-1)^{k}a_1\otimes\cdots\otimes a_{n-1}\pi(a_k).$$ %comentar que a esto se llega en varios pasos previos, tal como comento en el trabajo
		...
		\item<3-> The quasi-isomorphism $\phi:C_*(|N(PaB)|)\to C_*(O_A)$ is produced by the morphism $\Q(PaB)\to O_A$ which is trivial on objects and on morphisms is defined by $\phi(x)=\exp(t_{12}/2)$ and $\phi(i)=\Phi\in A_3$ is a \textbf{Drinfeld associator}.  %decir que O_A es esencialmente A vista como categoría Q-aditiva, es decir, los morfismos forman un Q-e.v.
		\item<4-> An associator is an element $\Phi\in A_3$ which is interpreted as the isomorphism $(A\otimes B)\otimes C\xrightarrow{\Phi} A\otimes (B\otimes C)$ and therefore satisfies some axioms related to this isomorphism. %comentar que son referidos a qué ocurre cuando cambia el orden de los objetos y a las formas de asociar 4 objetos.
		 %SI ME DA TIEMPO HABLAR MÁS DE ESTO
	\end{itemize}
\end{frame}
