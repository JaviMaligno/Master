...
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
\clip(-4.,-6.1) rectangle (13.293333333333337,5.7);
...
\begin{defi}
Two algebras $A$ and $B$ are called homotopy equivalent if there exists a chain (or zig-zag) of quasi-isomorphisms
\[
...
\begin{itemize}
	...
	\begin{figure}[h!]
		\includegraphics[scale=0.8]{Imagenes/extension}
	\end{figure}
	
	\item \emph{Cabling operations}: let $d_iB = d^n_i B$ for $1 ≤ i ≤ n$ be the parenthesized braid obtained
from $B$ by doubling its $i$-th strand (counting at the bottom), taking the ends of the
resulting “daughter strands” as an inner-most product:
\begin{figure}[h!]
		\includegraphics[scale=0.8]{Imagenes/cabling}
	\end{figure}
	
	\item \emph{Strand removal operations}: Let $s_iB = s^n_i B$ for $1 ≤ i ≤ n$ be the parenthesized braid
obtained from $B$ by removing its $i$-th strand (counting at the bottom):
	\begin{figure}[h!]
		\includegraphics[scale=0.8]{Imagenes/removal}
	\end{figure}
	
\end{itemize}
TENGO QUE HACER YO LOS DIBUJOS PERO HACIENDO QUE LAS TRENZAS VAYAN HACIA ABAJO, LO CUAL CAMBIA LA DE ELIMINAR (Y ALOMEJOR ALGUNA OTRA), SINO SERÍA INCOHERENTE CON LA INSERCIÓN
...
\end{defi}
An interpretation of these maps can be found in \cite{1deTamarkin}. We've chosen here to change the notation from the reference before to avoid confusion with the operations defined in section \ref{operations}, but the geometric interpretation shows that they are actually the same. This fact will help us showing that the map $\Q(PaB_*)\to O_A(*)$ is well defined.
...
\end{figure}
This morphism is nothing more than $d_1x$, so it is mapped via $\phi$ to $d^1\exp(t_{12}/2)$. On the other hand, by composition it can be expressed in several steps, moving only one strand or chaging the parenthesizing on each step as follows

DIBUJO QUE SALE DE LA DERECHA DEL HEXÁGONO
