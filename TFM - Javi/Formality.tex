...
\]
A VECES DIGO QUE EL GRADO ES 1 Y OTRAS $-1$, DEBERÍA SER CONSISTENTE (O EXPLICAR EN ALGÚN LAO QUE 1 ES PARA HACER REFERENCIA AL GRADO DE LA HOMOLOGÍA Y $-1$ PARA EL SIGNO)
To see that this map is well defined, we shall interpret the properties product and the Lie bracket in terms of element of the operads. Let $\mu(a,b)=a\cdot b$ and $l(a,b)=(-1)^{|a|}[a,b]$. Since both $\mu$ and $l$ are operations in arity 2, we have two insertion maps $\circ_1$, $\circ_2$ corresponding to introduce an element in the first or the second argument respectively. The action of the symmetric group consists of rearranging the arguments according to each permutation.
...
\]
so $l=(12)l$. Now, let us transcript the Leibniz identity to the operadic language. We have, 
\[
...
\item $\Phi$ is \emph{group-like}: $\Delta(\Phi)=\Phi\otimes\Phi$. %group-like elements form a group
\end{itemize}
\end{defi}
...
Since the operad $PaB_*$ is generated by $x$ and $i$, these conditions define $\phi$ uniquely. %CREO QUE LO GENERAN PORQUE SE ENTIENDE $X$ COMO PERMUTAR DOS CUALESQUIERA, Y A PARTIR DE ESO PUEDES HACER CUALQUIER MORFISMO TAMBIÉN, PORQUE FIJADA UNA PERMUTACIÓN, LAS DEMÁS ES MULTIPLICAR POR TRENZAS PURAS, QUE ES HACER ESA PERMUTACIÓN UN NÚMERO PAR DE VECES. 
