...
The complex $C_*^{\Q}(O_A(n))$ looks as follows. NO ME PARECE NADA EVIDENTE QUE SALGA LO QUE PONE A CONTINUACIÓN A PARTIR DE LA CONSTRUCCIÓN DEL ANTERIOR PÁRRAFO. The degree $k$ component $C_k^{\Q}(O_A(n))$ is $A_n^{\otimes k}$ CREO QUE ES PORQUE CADA ESPACIO VECTORIAL SIMPLICIAL ESTÁ GENERADO POR CADENAS DE MORFISMOS DE LONGITUD FIJA, QUE ES LO MISMO QUE COGER TUPLAS and the boundary map is given by
\[
...
COMENTAR LA PRUEBA REFERENCIÁNDOLA SIN DETALLE, ANTES DEFINIR LOS DRINFELD ASSOCIATOR \url{https://ncatlab.org/nlab/show/Drinfeld+associator} CREO QUE GROUPLIKE ES SIMPLEMENTE INVERTIBLE (LO MALO ES QUE NO LO ENTIENDO, OJALÁ TENGA QUE VER CON ESTO \url{https://en.wikipedia.org/wiki/Associator} ) 

AQUÍ SE DEFINE \url{https://en.wikipedia.org/wiki/Quasi-bialgebra}

LA PRUEBA TERMINA CUANDO SE DICE QUE ES BIYECTIVA
\end{proof}
...
The map $\phi$ produces a map of operads $C_*(PaB_*)=C_*^\Q(\Q(PaB_*))\to C_*(O_A(*))$. It is
well known that the homology operad of $C_*(PaB_*)$ is $e_2$ LA NOTACIÓN because $PaB_*$ induces a little disks operad as we saw in \ref{pab}. It is easy to check that $\phi$ is a
quasi-isomorphism for $*=2$ A VER SI VEO ESTO and hence it is a quasi-isomorphism of operads (since
$e_2$ NOTACIÓN is generated by $e_2(2)$ NOTACIÓN). By Proposition \ref{4.1}, $k$ is a quasi-isomorphism. Thus, the
chain operad $C_*(PaB_*)$ is quasi-isomorphic to $e_2$ ESA NOTACIÓN.
