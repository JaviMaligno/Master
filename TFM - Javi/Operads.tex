...
From now on, by ``disk'' we will mean ``open disk''. 
 ...
 There is an obvious homotopy equivalence $E_2(n)\simeq \coprod_{\sigma\in\Sigma_n}\mathcal{M}_2(n)$ from $E_2(n)$ to the disjoint union configuration spaces of $n$ points of $D^2$. This homotopy equivalence is given by ``forgetting the radius'' of each little disk. PONER AQUÍ LA HOMOLOGÍA CUANDO LA SEPA: VIENE EN EL ARTÍCULO QUE TENGO COMO HOMOLOGYOFLITTLEDISKS, ES COMPLICADA EN GENERAL, ASÍ QUE PONER SOLO LA DEL CASO $N=2$ QUE ES EL QUE ME HACE FALTA LUEGO (CON LA PRUEBA), PERO ESTO YA DESPUÉS DE LA HOMOTOPÍA %Hence $\dim H_0(E_2(n))=\dim H_1(E_2(n))=n!$ and $H_i(E_2(n))=0$ for $i>1$.
  The quotient $E_2(n)/\Sigma_n$ is then homotopy equivalent to a single copy of the configuration space. Hence $\pi_1(E_2(n))\cong PB_n$, the pure braid group of order $n$, and $\pi_i(E_2(n))=0$ for $i>1$. AUNQUE HAYA VARIAS COPIAS, EL GRUPO FUNDAMENTAL DEPENDE DEL PUNTO BASE (PONER QUIZÁ ALGO DE QUE SI NO SE TIEN EN CUENTA LA NUMERACIÓN ES EL CASO NO-EQUIVARIANTE)
...
Note that giving a morphism $O\to \xi_X$ is equivalent to giving a sequence of maps $O(k)\times X^k\to X$, so this definition is consistent with saying that $\Omega^2(X)$ is an $E_2$-algebra. 
