...
PROBABLEMENTE TENDRÍA QUE PONER QUE LOS ESPACIOS SON COMPACTLY GENERATED, QUE CREO QUE VIENE EN MAY, Y AQUÍ SE VERIFICA LA PROPOSICIÓN QUE COGÍ DE SIMPLICIAL
...
(LAX) MONOIDAL FUNCTORS \url{https://ncatlab.org/nlab/show/monoidal+functor} SI ME LO PUEDO AHORRAR ME LO AHORRO
...
\] 
The reader is referred to page 194 of \cite{Yau} to see the diagrams associated to the axioms. We won't need to show them explicitly since in most cases one can just mimic definition \ref{operadtop}. 
\end{enumerate}
...
\subsection{Connection between $E_2$ and Gerstenhaber algebras}

Now that we've defined operads in a more general context, we're ready to draw the connection between the $E_2$ operad and the Gerstenhaber algebras. Since $E_2=\{E_2(n)\}_{n\geq 0}$ is a collection of topological spaces, one can consider the collection of singular chain complexes $C_*(E_2)=\{C_*(E_2(n))\}_{n\geq 0}$ over a fixed ring $k$ (for our purposes $k=\Q$). We're going to show that this family is an operad of chain complexes making use of the Eilenber-Zilber map \cite{EZ} $C_*(X)\otimes C_*(Y)\to C_*(X\times Y)$ for arbitrary topological spaces $X$ and $Y$. 

\begin{prop}
PROBAR EN ESTA PARTE LO DE EILENBERG-ZILBER Y YA LUEGO DECIR QUE ESTABA AQUÍ \url{https://ncatlab.org/nlab/show/Eilenberg-Zilber+map} LO QUE PASA ES QUE HABLA DE GRUPOS SIMPLICIALES Y NO ESTOY SEGURO DE SI ESTO VALE
\end{prop}

This map passes to homology, endowing $H_*(E_2)$ with an structure of operad of graded $k$-modules, so we can consider a $H_*(E_2)$-algebra $A$. It is know that giving an $H_*(E_2)$-algebra is is equivalent to give a Gerstenhaber algebra \cite{cuentas} EN REALIDAD NO ESTOY SEGURO DE DÓNDE SE PRUEBA, PORQUE APARECE LO DE LAS MÓNADAS. We describe here how $A$ can be turned into a Gersternhaber algebra. Since $A$ is a $H_*(E_2)$-algebra, there is a sequence of maps $H_*(E_2(n))\otimes A^{\otimes n}\to A$. From this, we can obtain a commutative product of degree 0 given by the map
\[
...
\]
NECESITO REFERENCIA DE QUE ESTAS OPERACIONES GENERAN LAS DEMÁS DEL OPERAD
...
SÉ QUE ESTO NO ES TAN IMPORTANTE, PERO QUIERO SABER POR QUÉ CUMPLEN LAS PROPIEDADES DE PRODUCTO Y CORCHETE DE LIE
