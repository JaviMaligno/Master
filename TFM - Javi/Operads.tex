...
\draw (0.24666666666666592,3.493333333333331) node[anchor=north west] {$1$};
\draw (0.8733333333333326,3.52) node[anchor=north west] {$2$};
\draw (1.606666666666666,3.493333333333331) node[anchor=north west] {$3$};
\draw (2.9933333333333327,3.453333333333331) node[anchor=north west] {$n$};
\draw (2.153333333333333,3.453333333333331) node[anchor=north west] {$\cdots$};
\end{tikzpicture}
...
\item For a fixed field $k$, the category $\Vect_k$ of $k$-vector spaces carries the standard structure of a monoidal category coming from the tensor product (over $k$) of vector spaces. The standard braiding that identifies $V\otimes W$ with $W\otimes V$ by mapping elements $v\otimes w$ to $w\otimes v$ obviously makes $\Vect_k$ into a symmetric monoidal category. This is also true for the category $\Vectgr_k$ of graded vector spaces over $k$, though it is more common to introduce the Koszul sign convention in the braiding. 

\item For a commutative ring $k$, the category of $R$-modules and the category of $k$-algebras have obvious tensor structures that can be used in the graded case too (again, with the sign convention). %, but this tensor structure is not symmetric in general. Some cases where it is symmetric are $k=\Z_m$ and $k=\Q$. %en la tabla de caracteres de Z_2xZ_2 se ve que TRIVxAlT tiene que ser distinta a ALTxTRIV, como las representaciones son A-modulos para A el álgebra del grupo, se tiene lo que he dicho
...
\item The category of chain complexes of $k$-modules over a commutative ring $k$, $\Ch(k\Mod)$, can be endowed with a structure of symmetric monoidal category defining the following thensor structure. For chain complexes $X,Y\in\Ch(k\Mod)$ write $X\otimes Y$ for the chain complex whose component of degree $n$ is given by the direct sum
\[
...
\begin{enumerate}
\item Right actions of opposite symmetric groups $ρ_n^{op}:\Sigma_n^{op}→\Hom(\OO(n),\OO(n))$.
...
\[
\begin{tikzcd}
\OO(k)\otimes\left(\bigotimes_{i=1}^k\OO(n_i)\right)\otimes\left(\bigotimes_{r=1}^n\OO(j_r)\right)\arrow[dd, "\text{permute with the symmetry constrain}"']\arrow[r, "\gamma\otimes 1^j"] & 
...
\item  Compatibility with the action of the symmetric groups: $f\circ \rho_\OO(\sigma)=\rho_\mathcal{P}(\sigma)\circ f$ for every and $\sigma\in\Sigma_n$ and for all $n$, where $\rho_\OO$ and $\rho_\mathcal{P}$ are the actions of $\Sigma_n$ on $\OO$ and $\mathcal{P}$, respectively.
\end{enumerate}
\end{defi}
