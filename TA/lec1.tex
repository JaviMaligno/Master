...
Tendremos luego un cubo como el siguiente COPIAR EL CUBO O LAS CARAS POR SEPARADO, BUSCAR TIKZ COMMUTATIVE CUBE
...
Finalmente podemos probar que $i_*:H_q(X-Z,A-Z)\to H_q(X,A)$ es isomorfismo. Primero vemos que es sobreyectiva. Sea $[z]\in H_q(X,A)$, $z=\sum\lambda_i\sigma_i$ subordinado a $\{int A, X-\overline{Z}\}$, esto es, cualquier $\sigma_i$ cuya imagen caiga en $X-Z\supseteq X-\overline{Z}$. Entonces la imagen cae en $int(A)\subseteq A$, luego los podemos eliminar. Podemos suponer entonces que $X$ cae $X-Z$, luego $[z]$ define una clase  en $H_q(X-Z,A-Z)$. 
...
\section{Ejercicios}
\begin{ejer}
EL QUE TENGO POR ARRIBA PUESTO PARA HACER COMO EJERCICIO
\end{ejer}
