...
COPIAR LAS DIAPOSITIVAS SOBRE EL ANILLO DE COHOMOLOGÍA

Un ejemplo que no viene es $H^*(T;\Z)=H^*(S^1\times S^1;\Z)=\bigwedge_\Z[\alpha,\beta]$ por el ejemplo de superficie orientable. En general sale un álgebra exterior en más generadores. 

El isomorfismo de Anillos de productos viene de la propiedad universal del producto directo, usando las proyecciones $\pi_\alpha:\prod H^*\to H^*_\alpha$ y al $H$ va por otro lado la $(i_\alpha)^*$, además $\{i_\alpha^*\}([\varphi\smile\psi])=\{i^*_\alpha(\varphi)\smile i_\alpha^*(\psi)\}=\{i^*_\alpha(\varphi\smile\psi\}$ (esta es la aplicación que une el $H^*$ del $\coprod$ al $\prod$ de $H^*$). Para reducida igual con el wedge sum, pero para poder hacer la escisión los $x_\alpha$ tienen que ser retractos de deformación de un entorno suyo.

En el ejemplo de distinguir espacios, comprobar que tienen la misma cohomología (escribirlo después como ejercicio). Para ver que son distintos, el cup producto de una generador de la parte de $S^2$ consigo mismo se queda en esa parte, que en dimensión alta es 0. 

La unidad que viene de $R$ es la clase que genera la cohomología 0. 
