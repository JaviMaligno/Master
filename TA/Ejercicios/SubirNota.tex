...
%HACER EL CÁLCULO DE $H_1(S^3-\Ima g|_{\partial M})$ POR MAYER-VIETORIS CON LA SUBDIVISIÓN INDUCIDA POR LA BANDA DE MÖBIUS A VER SI SE PUEDEN CONECTAR, DEBE DE SALIR QUE EL HOMOMORFISMO ES MULTIPLICAR POR 2

%\textbf{Primera idea}
%
%La partición de $M$ que hemos hecho induce una partición en cuatro segmentos que agrupamos de 2 en 2 como se observa en la figura HACER DIBUJO
%
%Esta partición nos permite aplicar Mayer-Vietoris a $S^3-\partial M$ de forma similar al caso de $S^3-M$ mediante la descomposición $S^3-(N_1\cap N_2)=(S^3-N_1)\cup (S^3-N_2)$, de modo que $S^3-\partial M=(S^3-N_1)\cap (S^3-N_2)$. La inclusión $S^3-(M_1\cap M_2)\hookrightarrow S^3-(N_1\cap N_2)$ respeta las descomposiciones, luego la naturalidad de la sucesión de Mayer-Vietoris nos da el diagrama conmutativo
%\[
%\begin{tikzcd}[column sep=10]
%0\to H_2(S^3-(M_1\cap M_2))\arrow[r] \arrow[d] & H_1(S^3-M)\arrow[r]\arrow[d, "i_*"] & H_1(S^3-M-1)\oplus H_1(S^3-M_2)\arrow[r]\arrow[d] & 0\\
% 0\to  H_2(S^3-(N_1\cap N_2))  \arrow[r]                               & H_1(S^3-\partial M)\arrow[r] & H_1(S^3-N_1)\oplus H_1(S^3-N_2) \arrow[r] & H_1(S^3-(N_1\cap N_2))
% \end{tikzcd}
%\]
%Podemos calcular fácilmente $H_*(S^3-(N_1\cap N_2))$ combinando Mayer-Vietoris con la propsición 2B.1 o con el apartado (a), pues $S^3-(N_1\cap N_2)\cong S^3-\coprod_{i=1}^4 I$, de modo que $\widetilde{H}_i(S^3-(N_1\cap N_2))=0$ para $i\neq 2$ y $H_1(S^3-(N_1\cap N_2))\cong \Z^3$. Por el apartado (a) tenemos además que $\widetilde{H}_i(S^3-N_j)=0$ para $i\neq 2$ y $H_2(S^3-N_j)\cong\Z$ ($j=1,2$). Así que el diagrama anterior se simplifica a 
%
%\[
%\begin{tikzcd}
%\Z\arrow[r, "\cong"]\arrow[d] & \Z\arrow[d, "i_*"]\\
%\Z\oplus\Z\oplus\Z \arrow[r] & \Z
%\end{tikzcd}
%\]
%de donde necesariamente $i_*=0$. 
