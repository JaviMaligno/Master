...
A pesar de que el procedimiento que Padberg y Rao presentan consigue resolver el problema de la separación del poliedro del $b$-matching en tiempo polinómico, nosotros utilizaremos en nuestro análisis computacional una modificación del mismo. Tanto Padberg y Rao como Letchford, Reinelt y Theis en \cite{adam} basan su algoritmo para encontrar planos de cortes en el concepto de corte y arbol de Gomory-Hu. En esta sección vamos a detallar teóricamente y presentar el algoritmo que presentan los segundos autores para resolver el problema de separación para el poliedro del $b$-matching en tiempo polinómico.
...
Los cortes son importantes en teoría de grafos pues están estrechamente conectados con los problemas de flujo.  Gomory y Hu introdujeron en 1961 un algoritmo polinomial para encontrar una base de este espacio vectorial. Esta representación se conoce como el árbol de Gomory-Hu. Y especial por verificar la siguiente propiedad.
...
\end{prop}
\subsection{Desigualdades de blossom}
Como hemos comentado anteriormente y veremos más adelante, el poliedro del $b$-matching puede caracterizarse mediante desigualdades lineales de distintas maneras. Para este propósito, es conveniente caracterizarlo mediante las llamadas \textit{desigualdades de blossom}.
Edmonds en \cite{edmond} prueba el siguiente teorema.
\begin{thm}
...
A continuación presentamos el algoritmo para la separación del poliedro del matching
\begin{align*}
...
En esta sección presentamos los resultados del análisis de sensitividad del problema del emparejamiento biobjetivo sobre distintos grafos. Para los distintos valores de $\lambda$ hemos resuelto en Python 3.6 utilizando el software Gurobi 8.0 los correspondientes problemas de Programación Lineal. En primer lugar presentamos el código con el que hemos programado nuestra rutina de Padber-Rao:
\newline
\begin{pythone}
...
def padraomod(G,x):
    ...
    for e in GomHu.edges():
        ...
        # deseadas. 
        Fe = set([e for e in cutset if 
        1- H[e[0]][e[1]]['weight'] < H[e[0]][e[1]]['weight']])
