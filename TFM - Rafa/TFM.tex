...
Los las familias de contracción también son conocidas en la literatura como \textit{hypomatchable set} o \textit{shrinkable set} y Edmonds en \cite{edmon3} caracteriza estos conjuntos de la siguiente manera.
\begin{thm}
...
Supongamos que todas las aristas cuyos costes son alteras están en $\delta(v)$. Naturalmente, Si $c_{ij}'^{new} \geq 0$ $\forall (u,v) \in \delta(v)$, nuestro matching sigue siendo óptimo, luego podemos suponer que hay alguno que es negativo.
\begin{align*}
...
En el matching paramétrico, tal y como vimos en la Subsección 1.1.1, la función de coste es $c_{uv}(\lambda) = c^1_{uv} + \lambda c^2_{uv}$. Por tanto, para aplicar el procedimiento anterior, tenemos que considerar $c_{uv}^{new} = c^1_{uv} + (\lambda + \Delta) c^2_{uv}= c_{uv}(\lambda+\Delta)$. Para aplicar el procedimiento anterior necesitamos un subconunto $W\subset V$ tal que $E = \delta(W)\cup \gamma(W)$, es decir, el conjunto $W$ debe ser una cobertura por vértices de nuestro grafo $G$. Así, para cada $v\in W$ invocamos nuestro procedimiento para la reoptimización. 

Notemos que $\Delta$ no tendría por qué ser un parámetro real, sino que podríamos considerar $\Delta \in \R^{|E|}$. En ese caso, $c_{uv}^{new} = c^1_{uv} + (\lambda + \Delta_{uv}) c^2_{uv}$ 
...
$$
\max_{j:\overline{c^2}_j>0} -\frac{\overline{c}(\lambda)_j}{\overline{c^2}_j} \leq \Delta \leq \min_{j:\overline{c^2}_j<0} - \frac{\overline{c}(\lambda)_j}{\overline{c^2}_j}
$$
