...
\end{align*}
Esta formulación es ciertamente útil, pues tenemos una primera aproximación al problema del matching con una formulación propia de la Programación Lineal. Sin embargo veremos más adelante modificaciones del Teorema de Edmonds que nos darán algunas formulaciones equivalentes.
...
Más generalmente, si consideramos dos funciones de coste $c^1$ y $c^2$ cualesquiera y un parámetro $\lambda$ escalar, podemos formular el \textit{problema del emparejamiento biobjetivo de parámetro simple}
\begin{align*}
...
&x_{uv} \geq 0 \qquad \forall(u,v)\in E
\end{align*}
Denotamos simplemente $c(\lambda)_{uv} = c^1_{uv} + \lambda c_{uv}^2$. Este problema tiene una modificación natural si consideramos $\lambda \in \R^{|E|}$, en cuyo caso $c(\lambda)_{uv} = c_{uv}^1 \lambda_{uv}c^2_{uv}$. En este caso, hablamos del \textit{problema del emparejamiento perfecto de parámetro vectorial}.
...
En el matching paramétrico, tal y como vimos en la Subsección 1.1.1, la función de coste es en el caso de parámetro simple $c_{uv}(\lambda) = c^1_{uv} + \lambda c^2_{uv}$, y en el caso de parámetro vectorial $c_{uv}(\lambda) = c^1_{uv} + \lambda_{uv} c^2_{uv}$. Por tanto, para aplicar el procedimiento anterior, tenemos que considerar $c_{uv}^{new} = c^1_{uv} + (\lambda + \Delta\lambda) c^2_{uv}= c_{uv}(\lambda+\Delta\lambda)$. Para aplicar el procedimiento anterior necesitamos un subconunto $W\subset V$ tal que $E = \delta(W)\cup \gamma(W)$, es decir, el conjunto $W$ debe ser una cobertura por vértices de nuestro grafo $G$. Así, para cada $v\in W$ invocamos nuestro procedimiento para la reoptimización. 
...
\end{align*}
\subsection{Problema biobjetivo de parámetro simple}
...
\begin{align*}
\overline{c_R}(\lambda+\Delta\lambda)^t&= c_N(\lambda + \Delta\lambda)^t  - c_B(\lambda + \Delta\lambda)^t B^{-1}N \\
& = c_N(\lambda)^t  - c_B(\lambda)^t B^{-1}N + \Delta\lambda {c_N^2}^t  - \Delta\lambda {c_B^2}^t B^{-1}N \\
&= \overline{c_R}(\lambda)^t + \Delta\lambda \overline{c_R^2}^t \geq 0
\end{align*}
En este caso, sabemos que $B$ se mantiene como base óptima siempre que
$$
\max_{j:\overline{c^2}_j>0} -\frac{\overline{c}(\lambda)_j}{\overline{c^2}_j} \leq \Delta\lambda \leq \min_{j:\overline{c^2}_j<0} - \frac{\overline{c}(\lambda)_j}{\overline{c^2}_j}
$$
\subsection{Problema biobjetivo de parámetro vectorial}
En este caso consideremos $\lambda, \Delta\lambda \in \R^{|E|}$. Para el análisis denotamos, al igual que antes, $c(\lambda)_{uv} = c^1_{uv}+\delta_{uv}c^2_{uv}$ y sea además $c^2(\lambda)_{uv} = \lambda_{uv}c^2_{uv}$
\begin{align*}
\overline{c_R}(\lambda+\Delta\lambda)^t&= c_N(\lambda_N + \Delta\lambda_N)^t  - c_B(\lambda_B + \Delta\lambda_B)^t B^{-1}N \\
& = c_N(\lambda_N)^t  - c_B^2(\lambda_B)^t B^{-1}N + c_N(\Delta\lambda_N)^t  - c_B^2(\Delta\lambda_B)^t B^{-1}N \\
&=\overline{c_R}(\lambda)^t + \overline{c_R}^2(\Delta\lambda)^t \geq 0
\end{align*}
Cada variable no básica nos da una desigualdad sobre $\Delta\lambda$, de manera que tenemos un poliedro en $\R^{|E|}$ de valor admisibles, los cuales forman un poliedro.  
\newpage
