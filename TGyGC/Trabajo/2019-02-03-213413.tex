...
\only<2->{\begin{block}{Andipodal Polygon}
...
\end{block}}
\only<3->{\begin{block}{Thin Antipodal Polygon}
...
\begin{itemize}
\item<3-> Does a thick antipodal polygon always have larger area than a thin antipodal polygon?
\item<4-> How efficiently can one compute an antipodal polygon with minimal (maximal)
area?
\item<5-> What can be said about antipodal polygons in higher dimensions?
\end{itemize}
\end{block}}
...
\begin{frame}
\frametitle{Tritones}
\only<1-2>{A tritone is a musical interval
composed of three whole tones (six semitones).}
\vspace{1cm}

\only<2>{\emph{Hey, I'm no music theorist...}}
...
\begin{frame}
\frametitle{Results}
\only<1->{\begin{alertblock}{Claim 1}
...
\only<2->{\emph{Is there an exact analogue for thick antipodal polygons?}}
...
\only<2->{\begin{alertblock}{Theorem}
Every thin antipodal polygon on $S$ has less area than any non-thin antipodal
polygon on $S$.
\end{alertblock}}
\vspace{1cm}

\only<3->{\emph{Idea of the proof}: Induction on $n$ with lemmas 2 and 3 as base cases. Triangulate $S$. Choose wisely and antipodal pair $(p,p')$ that eliminates a thin triangle and use induction on $S\setminus\{p,p'\}$. Then analyse what are the possibilities when adding $p$ or $p'$ to a polygon.}
\end{frame}
...
reduced to solve $O(n)$ times the $(2(n - 1), n - 1)$-problem: 
\vspace{1cm}

\only<2->{In the $(2n, n - 1)$-problem an antipodal pair is not selected and can thus be removed
from the input. This approach gives a simple $O(n^{n-k+1})$ time algorithm for solving
the general $(2n, k)$-problem.}

\only<3->{\begin{alertblock}{Open Problem}
Can the $(2n, k)$-problem 
be solved in $O(n^k )$ time?
\end{alertblock}}
\end{frame}
