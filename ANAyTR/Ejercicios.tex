...
%Vamos con $\mathfrak{sp}(2l)$. Dada una matriz cuadrada de tamaño $2l$ escrita en bloques como $$A=\begin{pmatrix}
%m & n\\
%p & q
%\end{pmatrix}$$ imponemos que $[A,E]\in\mathfrak{sp}(2l)$ para toda matriz $E$ de la base. Denotamos $e_{ij}$ a la matriz construida como $E_{ij}$ pero de tamaño $l$. 
%
%\[
%\left[\begin{pmatrix}
%m & n\\
%p & q
%\end{pmatrix}, E_{i,i+l}\right]=\begin{pmatrix}
%0 & me_{ii} \\
%0 & ne_{ii}
...
%0 & 0
%\end{pmatrix}=\begin{pmatrix}
%p_{i\cdot} & m_{\cdot i}-q_{i\cdot}\\
%0 & p_{\cdot i}
%\end{pmatrix}
%\]
%Imponiendo que esta matriz esté en $\mathfrak{sp}(2l)$ tendríamos que $p_{i\cdot}=-p_{\cdot i}^t=-p_{i\cdot}$, luego $p=0$ (en particular $p^t=p$). Imponiendo $m_{\cdot i}-q_{i\cdot}=(m_{\cdot i}-q_{i\cdot})^t$ deducimos $$m_{\cdot i}-q_{i\cdot}=m_{i\cdot}-q_{\cdot i}$$ 
%de donde $-q_{i\cdot}=m_{i\cdot}$, luego $q=-m^t$ (salvo quizá en la diagonal). Continuando con los demás elementos de la base terminaríamos de obtener las condiciones buscadas. La misma estrategia es válida para $\mathfrak{o}(n)$. 
