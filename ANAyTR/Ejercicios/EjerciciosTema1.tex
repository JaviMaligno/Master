...
\end{ejercicio}
\begin{solucion}
Sea $V$ una representación irreducible cualquiera de $A$. Para $a,b\in A$ cualesquiera, tenemos $ab=ba$, luego para todo $v\in V$ tenemos $a(bv) =ab(v)=ba(v)=b(av)$
\[
\begin{tikzcd}
...
%\url{https://math.stackexchange.com/questions/556508/conjugate-class-in-the-dihedral-group}
...
%\url{http://math.webgirand.eu/pdf/dvp_agreg/Dn_en.pdf}
%\url{https://groupprops.subwiki.org/wiki/Number_of_conjugacy_classes}
%\url{https://groupprops.subwiki.org/wiki/Element_structure_of_dihedral_groups}
...
\end{align*}
por lo que $h=2$. Es decir, $V\otimes V\cong \C_+^+\oplus \C_+^-\oplus \C^2_2$. Cabe destacar el caso $N=3$, para el cual existe realmente $\C_2^2$, por lo que la única copia de dimensión 2 que se puede utilizar es $\C^2_1$, con la que de hecho coincidiría $\C_2^2$ si la definiéramos siguiendo el mismo patrón.
...
Compute the decomposition into irreducibles of all the representations of $A_5$ induced from all the irreducible representations of
\begin{enumerate}
\item $\Z_2$
\item $\Z_3$ 
\item $\Z_5$
...
\end{ejercicio}
\begin{solucion}
Comenzamos escribiendo la tabla de caracteres de $A_5$
\begin{center}
...
TRIV & 1 & 1 & 1 & 1 & 1\\
\hline
$\C^3_+$ & 3 & 0 &-1 & 	$\frac{1+\sqrt{5}}{2}$ & $\frac{1-\sqrt{5}}{2}$\\
\hline
$\C^3_-$ & 3 & 0 &-1 & 	$\frac{1-\sqrt{5}}{2}$ & $\frac{1+\sqrt{5}}{2}$\\
\hline
$\C^4$ & 4 & 1 & 0 & -1 & -1\\
...
\end{center}
\begin{enumerate}
\item Veamos el primer caso. Consideramos $\Z_2 \cong \{Id,(12)(34)\}$.
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
$\Z_2$ & Id & (12)(34) \\
...
ALT & 1 & -1\\
\hline
TRIV$\downarrow^{A_5}_{\Z_2}$		 & 1 & 1 \\
\hline
$\C^3_+\downarrow^{A_5}_{\Z_2}$ & 3  &-1 \\
\hline
$\C^3_-\downarrow^{A_5}_{\Z_2}$ & 3 & -1\\
\hline
$\C^4\downarrow^{A_5}_{\Z_2}$	& 4 & 0 \\
\hline
$\C^5\downarrow^{A_5}_{\Z_2}$ & 5 & 1  \\
\hline
\end{tabular}
\end{center}
Calculamos la tabla de productos escalares, de donde se deduce la descomposición de las representaciones inducidas como suma de irreps de $A_5$.
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
$Z\setminus W$ & TRIV & ALT \\
\hline
TRIV$\downarrow^{A_5}_{\Z_2}$		 & 1 & 0 \\
\hline
$\C^3_+\downarrow^{A_5}_{\Z_2}$ & 1  &2 \\
\hline
$\C^3_-\downarrow^{A_5}_{\Z_2}$ & 1 & 2\\
\hline
$\C^4\downarrow^{A_5}_{\Z_2}$	& 2 & 2 \\
\hline
$\C^5\downarrow^{A_5}_{\Z_2}$ & 3 & 2  \\
\hline
\end{tabular}
\end{center}
\item Veamos el segundo caso. Consideramos $\Z_3 \cong \gene{(123)}$. Sea $w$ una raíz cúbica primitiva de la unidad.
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
$\Z_3$ & Id & (123) & (132) \\
...
REP2 & 1 & $w^2$ & $w$\\
\hline 
TRIV$\downarrow^{A_5}_{\Z_3}$	& 1 & 1 & 1\\
\hline
$\C^3_+\downarrow^{A_5}_{\Z_3}$ & 3 & 0 & 0\\
\hline
$\C^3_-\downarrow^{A_5}_{\Z_3}$ & 3 & 0 & 0\\
\hline
$\C^4\downarrow^{A_5}_{\Z_3}$	& 4 & 1 & 1\\
\hline
$\C^5\downarrow^{A_5}_{\Z_3}$ 	& 5 & -1 & -1\\
\hline
\end{tabular}
\end{center}
Calculamos la tabla de productos escalares, de donde se deduce la descomposición de las representaciones inducidas como suma de irreps de $A_5$.
...
\begin{tabular}{|c|c|c|c|}
\hline
$Z\setminus W$ & TRIV & REP1 & REP2 \\
\hline
TRIV$\downarrow^{A_5}_{\Z_3}$		 & 1 & 0 & 0 \\
\hline
$\C^3_+\downarrow^{A_5}_{\Z_3}$ & 1  &1 & 1 \\
\hline
$\C^3_-\downarrow^{A_5}_{\Z_3}$ & 1 & 1 & 1\\
\hline
$\C^4\downarrow^{A_5}_{\Z_3}$	& 2 & 1 & 1 \\
\hline
$\C^5\downarrow^{A_5}_{\Z_3}$ & 1 & 2 & 2  \\
\hline
\end{tabular}
\end{center}
...
\item  Veamos el segundo caso. Consideramos $\Z_5 \cong \gene{(12345)}$. Sea $w=e^{2\pi i/5}$ una raíz quinta primitiva de la unidad. 
\begin{center}

\begin{tabular}{|c|c|c|c|c|c|}
\hline
$\Z_5$ & Id & (12345) & (13524) & (12453) & (15432) \\
\hline
TRIV & 1 & 1 & 1 & 1 & 1\\
\hline
REP1 & 1 & $w$& $w^2$ & $w^3$ & $w^4$ \\
\hline
REP2 & 1 & $w^2$ & $w^4$ & $w$ & $w^3$\\
\hline 
REP3 & 1 & $w^3$ &  $w$ & $w^4$ & $w^2$\\
\hline 
REP4 & 1 & $w^4$ & $w^3$ & $w^2$ & $w$\\
\hline 
TRIV$\downarrow^{A_5}_{\Z_5}$	& 1 & 1  					 &  1				  & 1  					& 1  					\\
\hline                                                                                                                                 
$\C^3_+\downarrow^{A_5}_{\Z_5}$ & 3 & $\frac{1+\sqrt{5}}{2}$ &  $\frac{1-\sqrt{5}}{2}$ & $\frac{1-\sqrt{5}}{2}$& $\frac{1+\sqrt{5}}{2}$\\
\hline                                                                                                                                 
$\C^3_-\downarrow^{A_5}_{\Z_5}$ & 3 & $\frac{1-\sqrt{5}}{2}$ &  $\frac{1+\sqrt{5}}{2}$	& $\frac{1+\sqrt{5}}{2}$& $\frac{1-\sqrt{5}}{2}$\\
\hline                                                                                                                                 
$\C^4\downarrow^{A_5}_{\Z_5}$	& 4 & -1					 &  -1					 & -1					& -1					\\
\hline                                                                                                                                 
$\C^5\downarrow^{A_5}_{\Z_5}$ 	& 5 & 0 					 &  0 						& 0 					& 0 					\\
\hline
\end{tabular}
\end{center}
Calculamos la tabla de productos escalares, de donde se deduce la descomposición de las representaciones inducidas como suma de irreps de $A_5$.
...
\begin{tabular}{|c|c|c|c|c|c|}
\hline
$Z\setminus W$ & TRIV & REP1 & REP2 & REP3 & REP4 \\
\hline
TRIV$\downarrow^{A_5}_{\Z_3}$		 & 1 & 0 & 0 & 0 & 0\\
\hline
$\C^3_+\downarrow^{A_5}_{\Z_3}$ & 1  &1 & 0 &0 &1\\
\hline
$\C^3_-\downarrow^{A_5}_{\Z_3}$ & 1 & 0& 1 & 1 &0\\
\hline
$\C^4\downarrow^{A_5}_{\Z_3}$	& 0 & 1 & 1 & 1&1\\
\hline
$\C^5\downarrow^{A_5}_{\Z_3}$ & 1 & 1 & 1  &1 &1\\
\hline
\end{tabular}
\end{center}

\item Veamos el tercer caso. Consideramos $A_4\cong\gene{(123),(12)(34)}$. Dada $w$ una raíz cúbica primitiva de la unidad, tenemos la siguiente tabla de caracteres. 
...
TRIV$\downarrow^{A_5}_{A_4}$	& 1 & 1 & 	1	&		1					\\
\hline                                                                                                                                 
$\C^3_+\downarrow^{A_5}_{A_4}$ & 3 & 0 &  0 & $-1$\\
\hline                                                                                                                                
$\C^3_-\downarrow^{A_5}_{A_4}$ & 3 &  0 &  0 & $-1$ \\
\hline                                                                                                                                 
$\C^4\downarrow^{A_5}_{A_4}$	& 4 & 1	&	1 & 		0		\\
\hline                                                                                                                              
$\C^5\downarrow^{A_5}_{A_4}$ 	& 5 & $-1$ 	& $-1$				 &  1				\\
\hline
\end{tabular}
\end{center}
...
\begin{tabular}{|c|c|c|c|c|}
\hline
$Z\setminus W$ & TRIV & REP1 & REP2 & REP3  \\
\hline
TRIV$\downarrow^{A_5}_{A_4}$		 & 1 & 0 & 0 & 0 \\
\hline
$\C^3_+\downarrow^{A_5}_{A_4}$ & 0  &0 & 0 &1 \\
\hline
$\C^3_-\downarrow^{A_5}_{A_4}$ & 0 & 0& 0 & 1 \\
\hline
$\C^4\downarrow^{A_5}_{A_4}$	& 1 & 0 & 0 & 1\\
\hline
$\C^5\downarrow^{A_5}_{A_4}$ & 0 & 1 & 1  &1 \\
\hline
\end{tabular}
\end{center}

\item Veamos el último caso. Consideramos $V_4=\Z_2\times\Z_2\cong\gene{(12)(34),(1 3)(2 4) }$. Tenemos la siguiente tabla de caracteres
...
\hline
TRIV$=$TRIV$\otimes$TRIV & 1 & 1 & 1 & 1 \\
\hline
REP1$=$TRIV$\otimes$ALT & 1 & $1$& $-1$ & $-1$  \\
\hline
REP2$=$ALT$\otimes$TRIV & 1 & $-1$ & $1$ & $-1$ \\
\hline 
REP3$=$ALT$\otimes$ALT & 1 & $-1$ &  $-1$ & $1$ \\
\hline 
TRIV$\downarrow^{A_5}_{V_4}$	& 1 & 1  					 &  1				  & 1  										\\
\hline                                                                                                                                 
$\C^3_+\downarrow^{A_5}_{V_4}$ & 3 & $-1$ & $-1$ & $-1$\\
\hline                                                                                                                                 
$\C^3_-\downarrow^{A_5}_{V_4}$ & 3 & $-1$ &  $-1$	& $-1$\\
\hline                                                                                                                                 
$\C^4\downarrow^{A_5}_{V_4}$	& 4 & 0					 &  0				 & 0										\\
\hline                                                                                                                                 
$\C^5\downarrow^{A_5}_{V_4}$ 	& 5 & 1 					 &  1						& 1										\\
\hline
\end{tabular}
\end{center}
...
$Z\setminus W$ & TRIV & REP1 & REP2 & REP3  \\
\hline
TRIV$\downarrow^{A_5}_{V_4}$		 & 1 & 0 & 0 & 0 \\
\hline
$\C^3_+\downarrow^{A_5}_{V_4}$ & 0  &1 & 1 &1 \\
\hline
$\C^3_-\downarrow^{A_5}_{V_4}$ & 0 & 1& 1 & 1 \\
\hline
$\C^4\downarrow^{A_5}_{V_4}$	& 1 & 1 & 1 & 1\\
\hline
$\C^5\downarrow^{A_5}_{V_4}$ & 2 & 1 & 1  &1 \\
\hline
\end{tabular}
\end{center}
\end{enumerate}
