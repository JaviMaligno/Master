...
Luego sii $j=kn$, tenemos que $[x,y^{kn}]=[x^{kn},y]=0$. Entonces el centro es $Z(A)_q = \{x^{kn}y^{k'n} \mid k, k' \in \Z\}$. Si $q$ no es una raíz de la unidad, entonces ningún monomio commuta y el centro es $\C$.
...
\[ \sum c_{ij} x^i y^j \]
y que $y^jv = \lambda^j v$ para todo $j \in \Z$, entonces $V$ está generado por monomios $\{x^iv \mid i \in \Z\}$. Por otro lado, por el problema $1.21$, existe un elemento central $\mu$ para $x^n$ tal que $x^nv=\mu v$.
Además:
\[ x^{-n+l}v = x^{l}x^{-n}v = x^l\mu^{-1} v \]
Por lo tanto, podemos ignorar también los monomios con exponentes negativos:
