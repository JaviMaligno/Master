...
Si $q$ no es raíz de la unidad, los únicos ideales bilaterales son los ideales $\{0\}$ y $A$, es decir, $A$ es un álgebra simple. Para probarlo, sea $I$ un ideal. Consideremos $p = p(x,y) = \sum a_{ij}x^i y^j$ de grado mínimo usando orden graded lexicographic order. Sea $h>0$ el grado en $x$ de este monomio. Si su grado es $0$, hemos acabado. Notemos que por minimalidad $p$ ha de tener término independiente. Ahora bien, es fácil comprobar que
$$
y^{-1}(x^i y^j)y = q^i x^i y^j
$$
Dado que $q$ no es raíz de la unidad, entonces $q^hp - y^{-1}py \in I$, es un polinomio en $x,y$ y tiene grado menor que $q$, lo cuál es una contradicción. Si $h=0$ se puede razonar análogamente con $x^{-1}px$.
