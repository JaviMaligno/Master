...
\begin{enumerate}[(a)]
\item
Si la característica del cuerpo $k$ es cero, entonces no hay representaciones de dimensión finita no triviales de $A$, ya que en tal representación $x, y$ deben actuar como matrices y la identidad $[x,y]=1$ no puede darse, porque el comutador de dos matrices tiene traza nula $Tr([x,y])=0$, pero $Tr(I)\neq 0$.

Es fácil provar por inducción que $y^jx = xy^j + jy^{j-1}$.
Supongase que $I$ es un ideal bilateral de $A$ que contiene un elemento no nulo $p(x,y)$ donde $x$ e $y$ aparecen con coeficientes no nulos.
...
Análogamente, usando que $x^jy = yx^j + jx^{j-1}$ llegamos a que siempre existe un polinomio $q$ que depeda sólo de $x$.
...
\begin{enumerate}[(a)]
\item[]
\item Denotemos $Z(A)_q$ el centro del álgebra de $q$-Weyl. Si $q=1$ entonces es claro que $Z(A)_1 = A$. Consideremos $k[u,v,u^{-1},v^{-1}]$ y la acción de $A$ dada por la extensión lineal de $x(u^iv^j) = u^{i+1}v^j$, $y(u^iv^j) = q^iu^i v^{j+1}$. Entonces, 
\begin{itemize}
\item Si $j=0$, entonces $x^i$ no puede estar en el centro, pues
$$
x^iy(1) = x^i(v) = u^iv \qquad yx^i(1) = y(u^i) = q^i u^iv
$$
\item Si $j\neq 0$ entonces $x^iy^j$ no pueden estar en el centro pues
$$
xx^i y^j (1) = u^{i+1}v^j \qquad x^i y^jx(1) = qu^{i+1}v^j
$$
\end{itemize}
Luego, siempre que $q$ no sea una raíz de la unidad, ningún monomio (no trivial) está en $Z(A)_q$, por lo que $Z(A)_q = \C$. Si $q$ es raíz $n$-ésima de la unidad (distinta de $1$), es claro que ningún monomio de la forma $x^i y^j$ con $j\neq 0$ puede conmutar, así como ningún monomio $x^i$ con $i\neq kn$ con $k\in Z$. Estos monomios, de hecho
$$
xx^i = x^i x \qquad x^{-1}x^i = x^ix^{-1}
$$
También conmutan con $y$, pues
$$
xy = qyx \Rightarrow x^2y = qxyx= q^2yx^2 \Rightarrow x^sy = q^s yx^s
$$
$$
xy = qyx \Rightarrow x^{-1}y = q^{-1}yx^{-1} \Rightarrow x^{-s}y = q^{-s}yx^{-s}
$$
Análogamente puede obtenerse que conmutan con $y^{-1}$. Por tanto, si $q$ es raíz $n$-ésima de la unidad, con $n$ el mínimo natural tal que $q^n =1$, entonces $Z(A)_q$ es el álgebra generada por $\{1,x^n\}$.
...
Si $q$ no es raíz de la unidad, los únicos ideales bilaterales son los ideales $\{0\}$ y $A$, es decir, $A$ es un álgebra simple.
La demostración de esto es un secreto bien guardado por el clique matemático ruso.
