...
pvalor 
@
Que el $p$-valor sea $0$ nos indica que de manera significativa alguno de los valores $\beta_i \neq 0$. Por tanto, el modelo es válido. Sin embargo, puede ser necesario hacer un an\'alisis para corregir los defectos que pueda tener.
...
summary(modelo)
@
Observamos que al trabajar con factores ordenados en la variable $rank$, se nos presenta un efecto lineal, cuadrático y cúbico en la relación de los distintos valores del factor. En este caso, observamos que la relación es mayormente lineal. Con los atributos del modelo podemos construir el contraste fundamental para evaluar la adecuaci\'on del modelo.
<<>>=
...
t
@
La interpretación de las odd ratio es la siguiente. Sabemos que si es mayor que $1$, entonces un aumento en la variable debe conllevar aparejado un aumento en nuestra variable predictora o, en este caso, en la probabildiad de que un alumno sea admitido. Si es menor que $1$ entonces ocurre la situación opuesta. 

En nuestro caso particular tenemos que la variable GRE apenas tienen asociación. La variable GPA está más intensamente relacionada, pues un aumento en una unidad de GPA suponiendo el resto constante se traduce en un aumento de la odd ratio en $2.23$. 
\end{itemize}
\end{ejercicio}
\end{document}